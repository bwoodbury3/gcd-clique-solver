...
def test_prime_factorization():
    assert primes.prime_factorization(20) == [2, 2, 5]
    assert primes.prime_factorization(412) == [2, 2, 103]
    assert primes.prime_factorization(8018367) == [3, 7, 31, 109, 113]
...
def _test_divisor_helper(num: int, factors: list[int]):
    ...
    divisors = primes.divisors_from_prime_factors(factors)
...
def test_divisors_from_prime_factors():
    ...
    assert primes.prime_factorization(product) == factors
...
def test_large_number_divisors():
    ...
    factors = primes.prime_factorization(num)
